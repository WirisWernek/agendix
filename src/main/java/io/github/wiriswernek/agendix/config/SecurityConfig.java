...
public class SecurityConfig {
	...
	public SecurityFilterChain securityFilterChain(
		...
		return http.csrf(AbstractHttpConfigurer::disable)
				.authorizeHttpRequests(customizer -> {
					customizer.requestMatchers("/hello-user").permitAll();
	...
	public UserDetailsService userDetailsService() {
		UserDetails commonUser = User.builder().username("common").password(passwordEncoder().encode("12345678"))
				.roles("USER").build();
		UserDetails adminUser = User.builder().username("admin").password(passwordEncoder().encode("wiris.123"))
